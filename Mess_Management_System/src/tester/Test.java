...
//ram yadav ram123@gmail.com ram123 pune 2022-01-09 8987738009 YEARLY 32000
					ValidateAll(sc.next(),sc.next(),sc.next(),sc.next(),sc.next(),sc.next(),
							sc.next(),sc.next(),sc.nextDouble(),Cusmap);
				...
				+ "C) According to date of registration\r\n"*/
					...
					Cusmap.values()
					.stream().sorted((c1,c2)->c1.getFname().compareTo(c2.getFname()))
					.forEach(System.out::println);
					
					System.out.println("\nSort Customer as per According to plan\n");
					Cusmap.values()
					.stream().sorted((c1,c2)->c1.getPln().compareTo(c2.getPln()))
					.forEach(System.out::println);
					System.out.println("\nSort Customer as per According to date of registration\n");
					Cusmap.values()
					.stream().sorted((c1,c2)->c1.getRegdate().compareTo(c2.getRegdate()))
					.forEach(System.out::println);
					break;
				case 5:
					/*5. Unsubscribe customer according plan duration (month wise (1, 3, 6, 12))*/
					...
					int dur = sc.nextInt();
					if(dur==1) {
						newList=CusList.stream().filter(s->s.getPln()==plan.MONTHLY).collect(Collectors.toList());
						   CusList.removeAll(newList);
					}
					else if(dur==3) {
						newList=CusList.stream()
						.filter(s->s.getPln()==plan.QUARTERLY)
						.collect(Collectors.toList());
						CusList.removeAll(newList);
					}
					else if(dur==6) {
						newList=CusList.stream()
						.filter(s->s.getPln()==plan.HALFYEAR)
						.collect(Collectors.toList());
						CusList.removeAll(newList);
					}
					else if(dur==12) {
						newList=CusList.stream()
						.filter(s->s.getPln()==plan.YEARLY)
						.collect(Collectors.toList());
						CusList.removeAll(newList);
					}
...
//					Cusmap.forEach((k,v)->System.out.println(v));
				...
				case 7:
					/*7) Modify all customers first name (make first 
					 * Letter capital of customers first name)*/
...
//8) Display email addresses of the customers who did registration in month of January
					...
					Cusmap.values()
					.stream().filter(s->s.getRegdate().getMonthValue()==1)
					.forEach(s->System.out.println(s.getEmail()+" "+s.getAddress()));
					break;
				case 10:
//10) Display count of the Customers who have register for plan: Monthly
					long count=Cusmap.values().stream()
					.filter(s->s.getPln()==plan.MONTHLY)
					.count();
					System.out.println("The count is : "+count);
					break;
				case 11:
//11) Search the Customer who lived in Akurdi.\r\n
					Cusmap.values().stream()
